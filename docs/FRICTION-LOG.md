...
- The Alexa+ docs require a bare 401 for anonymous calls; the Python MCP SDK's auth middleware returns 401 with `WWW-Authenticate` per RFC 9728, and its transport security guard returns 421 for any non-localhost Host. Neither is wrong; they disagree. The SDK's troubleshooting guide does document the 421 well (checked after the fact); the Alexa+ side does not say whether RFC 9728 headers are tolerated. Severity: medium. Suggestion (Alexa docs): state it; (Alexa toolkit examples): show a Streamable HTTP server behind a proxy with `allowed_hosts` set.

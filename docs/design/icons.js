// One stroked icon set for the whole product. No emoji: emoji render differently on
// every panel and read as clip-art at three metres.
const I = {
...
function icon(name, cls){
  // width/height as attributes, not CSS: an SVG without them falls back to 300x150 wherever
  // a stylesheet rule fails to reach it, and one stray icon ruins a whole screen.
  return `<svg class="${cls||''}" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="${I[name]}"/></svg>`;
}
function paint(){ document.querySelectorAll('[data-i]').forEach(e=>{ e.innerHTML = icon(e.dataset.i) + e.innerHTML; }); }
